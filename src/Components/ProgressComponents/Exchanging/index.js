...
const ExchangingComponent = ({ depositID }) => {
  const navigate = useNavigate();
  useEffect(() => {
    setTimeout(() => {
      navigate("/deposit/345?status=Sending");
    }, 5000);
  }, [navigate]);
  return (
    <div className="flex flex-col items-center gap-9 justify-center content-center w-full">
      <div className="flex flex-col flex-start self-stretch">
        ...
        <div className="text-[#D9D9D9] text-center text-[14px] lg:text-[22px]">
          Your coins are totally safe and getting swapped right now!
        </div>
      </div>
      <div
...
export default ExchangingComponent;
