...
const HomePage = () => {
  ...
  const onSwapButtonClicked = () => {
    ...
    service.postExchangeTransactionWithAxios({
      ...
      if (!result.error) {
        ...
        toast.warning(result.error ? result.error.description : "Request parameters are not valid");
      }
    }).catch(err => {
      console.log(err);
  ...
  useEffect(() => {
    if (currency_from && currency_to)
      range({ currency_from, currency_to });
          ...
          <div className="flex flex-col p-3 justify-center items-center gap-2.5 flex-1 self-stretch border-4 border-[#515151] text-white">
            <div className="flex lg:flex-row flex-col lg:justify-center items-start gap-[10px] lg:gap-[30px] self-stretch">
              ...
              <input
                ...
                placeholder="Enter your Address" value={address_to} onChange={(e) => setAddressTo(e.target.value)}
              />
            </div>
          </div>
